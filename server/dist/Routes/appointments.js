...
router.post('/appointments', authMiddleware_1.verifyToken, async (req, res) => {
    ...
    try {
        ...
        const serviceExists = await (0, db_1.query)('SELECT id FROM services WHERE id = $1', [services_id]);
        ...
        const checkConflict = await (0, db_1.query)('SELECT * FROM appointments WHERE appointments_date = $1 AND appointments_time = $2 AND status != $3', [appointments_date, appointments_time, 'CANCELLED']);
        if (checkConflict.rows.length > 0) {
            return res.status(400).json({ message: 'Este horario ya está reservado' });
        }
        // 2. Crear el turno
        const newAppointment = await (0, db_1.query)('INSERT INTO appointments (users_id, services_id, appointments_date, appointments_time) VALUES ($1, $2, $3, $4) RETURNING *', [users_id, services_id, appointments_date, appointments_time]);
        res.status(201).json({
...
router.get('/my-appointments', authMiddleware_1.verifyToken, async (req, res) => {
    const users_id = req.user.id; // Extraemos el ID del token verificado
    try {
        const myAppointments = await (0, db_1.query)(`SELECT 
        a.id, 
      ...
      WHERE a.users_id = $1
      ORDER BY a.appointments_date DESC, a.appointments_time DESC`, [users_id]);
